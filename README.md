...
- מוגן ב-Firebase Authentication - רק המשתמש שהוגדר כאדמין (לפי אימייל, ראו
  `isAdmin()` ב-`firestore.rules`) יכול לאשר/למחוק.
- הכניסה היא עם **כניסה עם גוגל** (כפתור "התחברות עם גוגל" בעמוד הכניסה של
  `admin.html`) - זו הדרך שבאמת פעילה. יש גם טופס אימייל/סיסמה ישן בעמוד,
  אבל בפועל לא הוגדר עבורו אף חשבון ב-Firebase Auth, אז הוא לא יעבוד -
  תמיד להשתמש בכפתור גוגל.
...
## SEO ושיתוף ברשתות חברתיות

לכל עמוד ציבורי (חוץ מ-`admin.html`, שמסומן `noindex`) יש כעת:

- `meta description` + `link rel="canonical"` ייעודיים.
- תגיות Open Graph ו-Twitter Card (כותרת/תיאור/תמונה) - כדי שכשמשתפים לינק
  לאתר (וואטסאפ, פייסבוק, ביו של אינסטגרם וכו') תופיע תצוגה מקדימה יפה עם
  תמונת מיתוג (`img/og-image.jpg`) במקום לינק ריק.
- `robots.txt` ו-`sitemap.xml` בשורש האתר, כדי שגוגל יוכל לסרוק ולאנדקס
  את העמודים (חוץ מ-`admin.html` שחסום ב-`robots.txt` וגם מסומן `noindex`).
- `manifest.json` + אייקונים (`img/icon-192.png`, `img/icon-512.png`,
  `img/apple-touch-icon.png`) - מאפשר "הוספה למסך הבית" בנייד כמו אפליקציה
  (עדיין בלי עבודה אופליין אמיתית - לזה צריך Service Worker, ראו למטה).

**חשוב**: אם הדומיין משתנה (למשל דומיין מותאם אישית במקום `yakmos.github.io`),
צריך לעדכן את הכתובת המלאה שמופיעה ב-`sitemap.xml`, ב-`robots.txt`, ובתגיות
ה-`canonical`/`og:url`/`og:image` בכל אחד מקבצי ה-HTML.

## מה אפשר להוסיף בהמשך

- חיפוש הפוך (הקלדה בעברית ומציאת המילה האנגלית).
- כתובת URL ייעודית לכל מילה (למשל `?word=astonishment`) - יאפשר גם דפי
  מילה עם meta תגיות ייחודיות לכל מילה, וגם שיתוף קישור ישיר למילה ספציפית.
...
- עבודה אופליין אמיתית (Service Worker) - כרגע יש רק manifest+אייקונים
  שמאפשרים התקנה למסך הבית, אבל בלי חיבור לאינטרנט האתר עדיין לא יעבוד.
